...
class CaptureGovernanceTests(unittest.TestCase):
    ...
    def human_presentation(self, assignment, receipt):
        return self.store.present(assignment_id=assignment["assignment_id"], capture_mode="physician_demo",
    ...
    def legacy_records(self):
        ...
        presentation = {"presentation_id": str(uuid4()), "presented_at": now(), "case_snapshot": snapshot,
                        "snapshot_sha256": snapshot_hash(snapshot), "supersedes_response_id": None}
        submitted = values(physician_code="LEGACY-QA")
        response = {"response_id": str(uuid4()), "presentation_id": presentation["presentation_id"],
                    "physician_code": "LEGACY-QA", "case_id": snapshot["case_id"], "case_family": snapshot["family_id"],
                    "case_version": snapshot["version"], "response_schema_version": "1.0", "case_snapshot": snapshot,
                    "snapshot_sha256": presentation["snapshot_sha256"], "presented_at": presentation["presented_at"],
                    "submitted_at": now(), "original_values": submitted,
                    "normalized_values": {key: value.strip() if isinstance(value, str) else value for key, value in submitted.items()},
                    "ai_advice_shown": False, "collection_purpose": "demo", "case_review_status": "unreviewed",
                    "eligible_for_study": False, "supersedes_response_id": None}
        with self.store.connection() as db:
            db.execute("INSERT INTO presentations VALUES (?, ?)", (presentation["presentation_id"], canonical_json(presentation)))
            db.execute("INSERT INTO responses VALUES (?, ?, ?, ?)",
                       (response["response_id"], presentation["presentation_id"], None, canonical_json(response)))
        return presentation, response

    def test_qa_requires_explicit_acknowledgment_and_retains_mode_after_restart(self):
        for changes in ({}, {"capture_mode": "fabricated_qa"},
                        {"capture_mode": "fabricated_qa", "qa_acknowledged": "true"},
                        {"capture_mode": "fabricated_qa", "qa_acknowledged": True, "permission_receipt_id": str(uuid4())}):
            with self.subTest(changes=changes), self.assertRaises(ValidationError):
                self.store.present(case_id=self.store.cases()[0]["case_id"], **changes)
        presentation = self.store.present(case_id=self.store.cases()[0]["case_id"],
                                          capture_mode="fabricated_qa", qa_acknowledged=True)
        ...
        self.assertEqual(self.store.lifecycle.overview()["records"][0]["permission_status"], "fabricated_qa")
    ...
    def test_human_metadata_exact_case_permission_and_retention_survive_restart(self):
        ...
        saved, duplicate = self.store.submit(presentation["presentation_id"], values(physician_code="PHY-QA"))
    ...
    def test_human_correction_preserves_receipt_original_values_and_retention_anchor(self):
        ...
        correction = self.store.present(supersedes_response_id=response["response_id"],
            capture_mode="physician_demo", permission_receipt_id=receipt["receipt_id"])
        corrected, _ = self.store.submit(correction["presentation_id"], values(physician_code="PHY-QA", next_action="Fabricated correction"))
        self.assertEqual(self.store.responses()[0], response)
        self.assertEqual(corrected["supersedes_response_id"], response["response_id"])
        for key in ("case_snapshot", "permission_receipt_id", "permission_version", "permission_sha256", "governance_id"):
            self.assertEqual(corrected[key], response[key])
        with self.store.connection() as db:
            self.assertEqual(self.store.lifecycle.metadata(db, corrected["response_id"]),
    ...
    def test_governance_revocation_and_new_plan_prevent_human_retries(self):
        ...
        self.governance.save(self.draft(based_on_governance_id=governance["governance_id"], status="revoked",
                                       approved_by_code="STF-QA", approval_note="Fabricated revocation."))
        with self.assertRaises(Conflict):
            self.store.submit(response["presentation_id"], values(physician_code="PHY-QA"))
        current = self.governance.overview()["current"]
        reopened, _ = self.governance.save(self.approved_body(based_on_governance_id=current["governance_id"]))
    ...
    def test_withdrawal_hides_response_export_and_blocks_pending_save_retry_correction(self):
        ...
        for identifier in (pending["presentation_id"], response["presentation_id"]):
            with self.subTest(identifier=identifier), self.assertRaises(Conflict):
                self.store.submit(identifier, values(physician_code="PHY-QA"))
        with self.assertRaises(Conflict):
            self.store.present(supersedes_response_id=response["response_id"], capture_mode="physician_demo",
                               permission_receipt_id=receipt["receipt_id"])
        with self.store.connection() as db:
            self.assertEqual(json.loads(db.execute("SELECT payload FROM responses").fetchone()[0]), response)
    ...
    def test_expiry_and_hold_hide_retained_content_from_ordinary_use(self):
        ...
        self.store.lifecycle.clock = lambda: (datetime.fromisoformat(expires) + timedelta(seconds=1)).isoformat()
        self.assertEqual(self.store.responses(), [])
        self.assertEqual(self.store.export()["responses"], [])
        with self.assertRaises(Conflict):
            self.store.submit(response["presentation_id"], values(physician_code="PHY-QA"))
        self.store.lifecycle.clock = original_clock
        self.store.lifecycle.hold({"request_id": str(uuid4()), "response_ids": [response["response_id"]],
                                  "actor_code": "STF-QA", "authority_record": "Fabricated hold authority."})
        self.assertEqual(self.store.responses(), [])
        self.assertEqual(self.store.export()["responses"], [])
        with self.assertRaises(Conflict):
            self.store.present(supersedes_response_id=response["response_id"], capture_mode="physician_demo",
    ...
    def test_legacy_correction_cannot_reclassify_original_or_invent_permission(self):
        ...
        for mode, acknowledgment in (("fabricated_qa", True), ("physician_demo", False)):
            with self.subTest(mode=mode), self.assertRaises(Conflict):
                self.store.present(supersedes_response_id=legacy["response_id"], capture_mode=mode,
                                   qa_acknowledged=acknowledgment)
        correction = self.store.present(supersedes_response_id=legacy["response_id"], capture_mode="legacy_unclassified")
        corrected, _ = self.store.submit(correction["presentation_id"], values(physician_code="LEGACY-QA"))
        ...
        with self.store.connection() as db:
            self.assertEqual(self.store.lifecycle.metadata(db, corrected["response_id"])["permission_status"], "unknown_legacy")
        with self.assertRaises(ValidationError):
            self.store.present(case_id=self.store.cases()[0]["case_id"], capture_mode="legacy_unclassified")
    ...
    def test_general_response_export_excludes_private_governance_registry(self):
        ...
        for private in (governance["operator"]["legal_name"], governance["operator"]["privacy_contact"],
                        governance["owners"][0]["person_name"], governance["owners"][0]["contact"]):
...
class CaptureGovernanceHttpTests(unittest.TestCase):
    ...
    def test_http_requires_explicit_mode_and_qa_acknowledgment(self):
        ...
        invalid = [{"case_id": case_id}, {"case_id": case_id, "capture_mode": "fabricated_qa",
                   "permission_receipt_id": None, "qa_acknowledged": False},
                   {"case_id": case_id, "capture_mode": "physician_demo", "permission_receipt_id": None,
    ...
    def test_http_approved_permission_copy_capture_and_decline_gate(self):
        ...
        status, _, body = self.request("POST", "/api/presentations", {
            "assignment_id": assignment["assignment_id"], "capture_mode": "physician_demo",
            "permission_receipt_id": receipt["receipt_id"], "qa_acknowledged": False})
        self.assertEqual(status, 201)
        response_body = {"presentation_id": json.loads(body)["presentation_id"], "values": values(physician_code="PHY-QA")}
        status, _, _ = self.request("POST", "/api/responses", response_body)
        self.assertEqual(status, 201)
        status, _, _ = self.request("POST", "/api/permissions", self.permission_body(governance, choice="decline"))
        self.assertEqual(status, 201)
        status, _, _ = self.request("POST", "/api/responses", response_body)
    ...
    def test_http_draft_or_forged_approval_cannot_issue_permission(self):
        ...
        self.assertEqual(status, 201)
        status, _, _ = self.request("POST", "/api/permissions", self.permission_body(json.loads(body)["governance"]))
        self.assertEqual(status, 409)
