...
class CollectionTests(unittest.TestCase):
    ...
    def protocol_body(self, **changes):
        body = {
            ...
            "title": "Synthetic software test plan",
            ...
            "consent_version": "TEST_ONLY",
            "retention_days": 7,
            "backup_owner_code": "TEST_BACKUP",
            "backup_frequency": "manual_before_changes",
            "backup_retention_days": 7,
            "notes": "Temporary test data only.",
        }
        body.update(changes)
        return body
    ...
    def test_protocol_retry_preserves_history_and_rejects_stale_edits(self):
        ...
        second, _ = self.store.collection.save_protocol(
            self.protocol_body(
                based_on_protocol_id=first["protocol_id"], title="Revised synthetic plan"
            )
        )
        with self.assertRaises(Conflict):
            self.store.collection.save_protocol(
                self.protocol_body(based_on_protocol_id=first["protocol_id"])
            )
        overview = Store(self.path).collection.overview()
    ...
    def test_consent_text_is_preserved_exactly_and_whitespace_is_incomplete(self):
        ...
        protocol, _ = self.store.collection.save_protocol(
            self.protocol_body(consent_statement=statement, consent_version=version)
        )
        self.assertEqual(protocol["consent_statement"], statement)
        self.assertEqual(protocol["consent_version"], version)
        self.assertEqual(
            self.store.export()["collection_protocols"][0]["consent_statement"], statement
        )
        self.store.collection.save_protocol(
            self.protocol_body(
                based_on_protocol_id=protocol["protocol_id"],
                consent_statement=" \n\t ",
                consent_version=" ",
            )
        )
    ...
    def test_concurrent_protocol_edits_cannot_fork_history(self):
        ...
        bodies = [
            self.protocol_body(based_on_protocol_id=first["protocol_id"], notes=note)
    ...
    def test_invalid_protocols_cannot_enable_study_or_record_invalid_rules(self):
        invalid = (
            ...
            {"backup_frequency": "automatic_cloud"},
            {"physician_codes": ["DEMO_01", "DEMO_01"]},
            {"physician_codes": ["name@example.test"]},
            {"owner_code": "bad code"},
        )
    ...
    def test_assignment_pins_rules_version_and_code_through_response_correction(self):
        ...
        corrected, _ = self.store.submit(
            correction["presentation_id"], values(next_action="Corrected")
        )
        ...
        self.store.catalog.add_review(
            {
                ...
                "comments": "Synthetic software test only.",
    ...
    def test_verified_backup_restores_all_data_into_separate_database(self):
        ...
        for key in ("current", "history", "assignments", "readiness"):
            self.assertEqual(
                restored.collection.overview()[key], self.store.collection.overview()[key]
            )
