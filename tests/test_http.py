...
class HttpTests(unittest.TestCase):
    ...
    def present(self):
        ...
        status, _, body = self.request("POST", "/api/presentations", qa_presentation(case_id=case_id))
    ...
    def test_rejects_bad_host_origin_and_missing_write_guard(self):
        ...
        for origin in ("https://evil.example", "null", "http://127.0.0.1:1"):
            with self.subTest(origin=origin):
                status, _, _ = self.request("POST", "/api/presentations", {},
                                            headers={"Origin": origin})
                self.assertEqual(status, 403)
        status, _, _ = self.request("GET", "/api/responses",
                                    headers={"Sec-Fetch-Site": "cross-site"})
        self.assertEqual(status, 403)
        status, _, _ = self.request("POST", "/api/presentations", {}, write_headers=False,
    ...
    def test_rejects_bad_content_type_oversize_or_ambiguous_json(self):
        ...
        for payload in (b"{", b"[]", b'{"case_id":"a","case_id":"b"}',
                        b'{"case_id":NaN}', b'{"case_id":Infinity}', b"\xff"):
    ...
    def test_full_length_unicode_response_is_saved_exactly(self):
        presentation = self.present()
        submitted = values(next_action="🧪" * 5000, next_information="🔎" * 5000,
                           decision_change="📝" * 5000, rationale="💬" * 5000)
        raw = json.dumps({"presentation_id": presentation["presentation_id"],
                          "values": submitted}, ensure_ascii=False).encode("utf-8")
    ...
    def test_duplicate_keys_in_response_values_are_rejected_without_saving(self):
        presentation = self.present()
        raw = ('{"presentation_id":' + json.dumps(presentation["presentation_id"]) +
               ',"values":' + json.dumps(values())[:-1] +
               ',"next_action":"Ambiguous second value"}}').encode("utf-8")
        status, _, body = self.request("POST", "/api/responses", raw=raw)
        self.assertEqual(status, 400)
        self.assertTrue(json.loads(body)["error"])
        self.assertEqual(self.store.responses(), [])
    ...
    def test_validation_missing_resources_and_conflict_have_clear_errors(self):
        ...
        self.assertTrue(json.loads(body)["error"])
        self.assertEqual(self.store.responses(), [])
        status, _, _ = self.request("POST", "/api/presentations", qa_presentation(case_id="missing"))
    ...
    def test_failed_persistence_returns_error_without_false_success_or_response_text(self):
        ...
        with patch("twin_lab.store.sqlite3.connect", side_effect=readonly_connection):
            with self.assertLogs(level="ERROR") as logs:
                status, _, body = self.request("POST", "/api/responses", {
    ...
    def test_new_review_version_plan_assignment_and_backup_routes_round_trip(self):
        ...
        review = {"request_id": str(uuid4()), "version_id": version["version_id"],
                  "reviewer_code": "TEST", "reviewed_on": date.today().isoformat(),
                  "comments": "Software test only.", "disposition": "approved",
                  "supersedes_review_id": None}
        snapshot = dict(version["snapshot"], version="test-http-2", narrative="Synthetic revised narrative.")
        revision = {"request_id": str(uuid4()), "based_on_version_id": version["version_id"],
                    "version": "test-http-2", "editor_code": "TEST", "change_note": "Software test.",
                    "snapshot": snapshot}
        protocol = {"request_id": str(uuid4()), "based_on_protocol_id": None, "title": "Test draft",
                    "owner_code": "", "physician_codes": ["DEMO_01"], "consent_statement": "",
                    "consent_version": "", "retention_days": None, "backup_owner_code": "",
                    "backup_frequency": "manual_before_changes", "backup_retention_days": None, "notes": ""}
        saved = {}
        for route, key, payload in (("case-reviews", "review", review),
        ...
        assignment = {"request_id": str(uuid4()), "protocol_id": saved["protocol"]["protocol_id"],
                      "physician_code": "DEMO_01", "version_id": version["version_id"], "notes": ""}
        status, _, raw = self.request("POST", "/api/assignments", assignment)
        self.assertEqual(status, 201)
        assigned = json.loads(raw)["assignment"]
        status, _, raw = self.request("POST", "/api/presentations", qa_presentation(assignment_id=assigned["assignment_id"]))
        self.assertEqual(status, 201)
        presentation = json.loads(raw)
        self.assertEqual(presentation["case_review_id"], saved["review"]["review_id"])
        status, _, raw = self.request("POST", "/api/responses",
        ...
        self.assertEqual(set(overview), {"current", "history", "assignments", "readiness", "backups"})
    ...
    def test_new_mutations_reject_missing_guard_and_forged_study_fields(self):
        for route in ("case-reviews", "case-versions", "protocols", "assignments", "backups"):
            with self.subTest(route=route):
                status, _, _ = self.request("POST", "/api/" + route, {},
                    write_headers=False, headers={"Content-Type": "application/json"})
                self.assertEqual(status, 403)
                status, _, raw = self.request("POST", "/api/" + route,
