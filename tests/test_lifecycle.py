...
class LifecycleTests(unittest.TestCase):
    def setUp(self):
        ...
        self.governance = {
            ...
            "operator": {"pilot_close_date": "2026-09-30"},
            "retention": {
                "version": "TEST_ONLY",
                "response_days": 90,
                "after_close_days": 30,
    ...
    def response(self, previous=None, governed=True, submitted_at=None, code="TEST_CODE"):
        ...
        record = {
            ...
            "physician_code": code,
            "submitted_at": submitted_at or self.timestamp,
            "presented_at": self.timestamp,
            "case_id": case["case_id"],
            "case_snapshot": case,
            "original_values": values(physician_code=code),
        ...
        with self.store.connection() as db:
            ...
            db.execute(
                ...
                (presentation_id, canonical_json(presentation)),
            )
            db.execute(
            ...
            self.lifecycle.attach(
                db,
                record,
                self.governance if governed else None,
                str(uuid4()) if governed else None,
            )
        return record

    def withdrawal(self, code="TEST_CODE"):
        return self.lifecycle.withdraw(
            {"request_id": str(uuid4()), "physician_code": code, "actor_code": "TEST_CUSTODIAN"}
        )[0]

    def verify(self, withdrawal, days=None):
        return self.lifecycle.verify_withdrawal(
            {
                "request_id": str(uuid4()),
                "withdrawal_id": withdrawal["withdrawal_id"],
                "actor_code": "TEST_CUSTODIAN",
    ...
    def test_expiry_uses_original_anchor_and_earlier_pilot_deadline(self):
        self.assertEqual(
            expiry(self.timestamp, self.governance), "2026-10-30T23:59:59.999999+00:00"
        )
        self.assertEqual(
            expiry("2026-01-01T00:00:00+00:00", self.governance), "2026-04-01T00:00:00+00:00"
        )
        first = self.response()
        correction = self.response(previous=first, submitted_at="2026-10-01T12:00:00+00:00")
        with self.store.connection() as db:
            self.assertEqual(
                self.lifecycle.metadata(db, first["response_id"]),
    ...
    def test_verification_does_not_extend_expiry_and_legacy_requires_explicit_days(self):
        ...
        self.assertEqual(
            verified["deadlines"][response["response_id"]], "2026-09-13T00:00:00+00:00"
        )
    ...
    def test_hold_blocks_normal_use_and_purge_until_authorized_release(self):
        ...
        hold, _ = self.lifecycle.hold(
            {
                "request_id": str(uuid4()),
                "response_ids": [response["response_id"]],
                "actor_code": "TEST_STAFF",
                "authority_record": "Synthetic authority reference",
            }
        )
        ...
        self.lifecycle.release_hold(
            {
                ...
                "actor_code": "TEST_STAFF",
                "authority_record": "Synthetic release reference",
            }
        )
        self.assertEqual(
            self.lifecycle.plan_disposal()["eligible_root_ids"], [response["response_id"]]
        )
    ...
    def test_disposal_removes_whole_chain_and_pending_correction_presentations(self):
        ...
        self.assertEqual(
            ledger_entries(self.lifecycle.ledger_path)[0]["root_ids"], [original["response_id"]]
        )
    ...
    def test_failed_database_purge_rolls_back_and_journal_blocks_reuse(self):
        ...
        with self.store.connection() as db:
            db.execute(
                "CREATE TRIGGER test_disposal_failure BEFORE DELETE ON responses BEGIN SELECT RAISE(ABORT,'test'); END"
            )
        ...
        with self.store.connection() as db:
            ...
            self.assertEqual(
                self.lifecycle.restriction(db, original["response_id"]), "disposal_recorded"
            )
            with self.assertRaises(sqlite3.IntegrityError):
    ...
    def test_old_backup_restore_also_reapplies_pending_withdrawal_and_hold(self):
        ...
        self.lifecycle.hold(
            {
                "request_id": str(uuid4()),
                "response_ids": [held["response_id"]],
                "actor_code": "TEST_STAFF",
                "authority_record": "Synthetic hold",
            }
        )
        ...
        with restored.connection() as db:
            self.assertEqual(
                restored.lifecycle.restriction(db, withdrawn["response_id"]), "withdrawn"
            )
    ...
    def test_backup_deadline_cannot_reset_source_expiry(self):
        ...
        backup, _ = self.store.collection.backup({"request_id": str(uuid4())})
        expected = min(
            instant(backup["created_at"]) + timedelta(days=30),
    ...
    def test_malformed_ledger_fails_closed_and_missing_ledger_cannot_restore(self):
        ...
        with self.assertRaises(ValidationError):
            restore_snapshot(
                self.path, self.path.parent / "restore.sqlite3", self.path.parent / "missing.jsonl"
            )
    ...
    def test_disposal_without_durable_ledger_cannot_delete_response(self):
        ...
        with patch(
            "twin_lab.lifecycle_files.append_ledger", side_effect=OSError("Synthetic disk full")
        ):
    ...
    def test_failed_withdrawal_database_write_still_blocks_use_and_retries_safely(self):
        ...
        with self.store.connection() as db:
            db.execute(
                "CREATE TRIGGER test_event_failure BEFORE INSERT ON lifecycle_events BEGIN SELECT RAISE(ABORT,'test'); END"
            )
        with self.assertRaises(sqlite3.IntegrityError):
    ...
    def test_hold_retry_after_release_and_disposal_keeps_original_record(self):
        ...
        body = {
            ...
            "response_ids": [response["response_id"]],
            "actor_code": "TEST_STAFF",
            "authority_record": "Synthetic authority",
        }
        ...
        self.lifecycle.release_hold(
            {
                ...
                "actor_code": "TEST_STAFF",
                "authority_record": "Synthetic release",
            }
        )
        self.lifecycle.apply_disposal(self.lifecycle.plan_disposal(), "TEST_STAFF")
    ...
    def test_structurally_invalid_journals_fail_with_safe_errors(self):
        self.response()
        for path, payload in (
            (self.lifecycle.ledger_path, '{"root_ids":null}'),
    ...
    def test_retried_disposal_cannot_restart_backup_grace(self):
        ...
        for timestamp in ("2026-08-01T00:00:00+00:00", "2026-09-10T00:00:00+00:00"):
            append_ledger(
                ...
                {
                    "entry_id": str(uuid4()),
                    "root_ids": [response["response_id"]],
                    "deleted_at": timestamp,
                    "actor_code": "TEST_STAFF",
                    "plan_confirmation": "Synthetic fixture",
                },
            )
        manifest = next(
            row
            for row in self.lifecycle.overview()["backups"]
            if row["backup_id"] == backup["backup_id"]
        )
    ...
    def test_verify_restored_withdrawal_preserves_scope_when_snapshot_predates_some_roots(self):
        ...
        restore_snapshot(
            self.path.parent / "backups" / backup["filename"],
            destination,
            self.lifecycle.ledger_path,
        )
        restored = Store(destination)
        restored.lifecycle.clock = lambda: self.timestamp
        verified, _ = restored.lifecycle.verify_withdrawal(
            {
                "request_id": str(uuid4()),
                "withdrawal_id": withdrawal["withdrawal_id"],
                "actor_code": "TEST_STAFF",
        ...
        self.assertEqual(
            verified["deadlines"], {present["response_id"]: "2026-10-10T12:00:00+00:00"}
        )
        self.assertEqual(
            restored.lifecycle.overview()["withdrawals"][0]["root_ids"], withdrawal["root_ids"]
        )
    ...
    def test_verify_pending_withdrawal_after_one_chain_has_already_expired_and_been_disposed(self):
        ...
        self.assertEqual(
            verified["deadlines"], {remaining["response_id"]: "2026-10-10T12:00:00+00:00"}
        )
        self.assertEqual(
            self.lifecycle.overview()["withdrawals"][0]["root_ids"], withdrawal["root_ids"]
        )
    ...
    def test_later_restore_does_not_restart_verified_withdrawal_clock_for_previously_absent_root(
        self,
    ):
        ...
        self.assertEqual(
            due["eligible_root_ids"], sorted([first["response_id"], second["response_id"]])
        )
    ...
    def test_later_legacy_restore_uses_only_explicit_period_from_original_verification(self):
        _, second, original, restored = self.restored_after_partial_verification(
            governed=False, days=7
        )
        self.assertEqual(original["withdrawal_days"], 7)
        self.assertNotIn(
            second["response_id"],
            restored.lifecycle.plan_disposal("2026-09-17T11:59:59+00:00")["eligible_root_ids"],
        )
        self.assertIn(
            second["response_id"],
            restored.lifecycle.plan_disposal("2026-09-17T12:00:00+00:00")["eligible_root_ids"],
        )
