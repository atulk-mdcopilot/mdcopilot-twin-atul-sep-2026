...
def _lifecycle_manifest(db, created_at):
    ...
    responses = {
        row[0]: json.loads(row[1]) for row in db.execute("SELECT id,payload FROM responses")
    }
    roots = sorted(
        identifier for identifier, row in responses.items() if not row["supersedes_response_id"]
    )
    has_metadata = db.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='response_lifecycle'"
    ).fetchone()
    metadata = (
        {
            row[0]: json.loads(row[1])
            for row in db.execute("SELECT id,payload FROM response_lifecycle")
        }
        if has_metadata
        else {}
    )
    ...
    for root in roots:
        ...
        if record and record.get("policy"):
            ...
            deadlines.extend(
                (
                    instant(created_at) + timedelta(days=policy["backup_days"]),
                    instant(record["expires_at"])
                    + timedelta(days=policy["backup_after_deletion_days"]),
                )
            )
    return {
        "source_root_ids": roots,
        "source_backup_grace_days": grace,
...
def create_backup(store, body):
    ...
    with store.connection() as gate:
        ...
        existing = gate.execute(
            "SELECT payload FROM local_backups WHERE id = ?", (request_id,)
        ).fetchone()
        if existing:
            ...
            if (
                ...
                raise RuntimeError(
                    "The saved backup file is missing or changed. Create a new backup."
                )
            return record, True
        if not path.exists():
            fd, temporary_name = tempfile.mkstemp(
                prefix=".snapshot-", suffix=".sqlite3", dir=directory
            )
            os.close(fd)
        ...
        gate.execute(
            "INSERT INTO local_backups(id, payload) VALUES (?, ?)",
