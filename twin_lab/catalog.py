...
from .catalog_schema import create_tables, matched_variant, review_values, version_values
from .schemas import (CASE_FLAGS, Conflict, NotFound, ValidationError, canonical_json,
                      now, read_json, snapshot_hash, validate_case, validate_id)

FAMILIES = Path(__file__).resolve().parent.parent / "fixtures" / "families.json"


def _get(db, table, identifier):
    # Table names are constant internal callers, never supplied by an HTTP request.
    row = db.execute(f"SELECT payload FROM {table} WHERE id = ?", (identifier,)).fetchone()
    if row is None:
        raise NotFound("The requested catalog record was not found.")
    return read_json(row[0])


def _latest(db, case_id):
    row = db.execute("SELECT payload FROM case_versions WHERE case_id = ? ORDER BY rowid DESC LIMIT 1",
                     (case_id,)).fetchone()
    if row is None:
...
def _latest_review(db, version_id):
    row = db.execute("SELECT payload FROM case_reviews WHERE version_id = ? ORDER BY rowid DESC LIMIT 1",
                     (version_id,)).fetchone()
    return read_json(row[0]) if row else None


def _retry(db, table, request_id, request_body):
    row = db.execute(f"SELECT request_body, payload FROM {table} WHERE request_id = ?",
                     (request_id,)).fetchone()
    if row is None:
        return None
    if row[0] != request_body:
        raise Conflict("This request ID was already used for different content. Reload before retrying.")
    return read_json(row[1])
...
class Catalog:
    ...
    def initialize(self, cases, include_families=True):
    ...
    def _seed_wording_revision(self, db):
        ...
        for row in sources:
            ...
            self._insert_version(db, snapshot, identifier, None,
                                 "Source wording revision: remove 'fictional' from the narrative and facts; "
                                 "synthetic provenance and clinical values retained.", source["version_id"])

    @staticmethod
    def _insert_version(db, snapshot, identifier, editor, note, based_on,
                        request_id=None, request_body=None):
        record = {
            "version_id": identifier, "case_id": snapshot["case_id"],
            "case_version": snapshot["version"], "snapshot": snapshot,
            "snapshot_sha256": snapshot_hash(snapshot), "created_at": now(),
            "editor_code": editor, "change_note": note, "based_on_version": based_on,
        }
        db.execute("""INSERT INTO case_versions
            (id, case_id, version_label, based_on_id, request_id, request_body, payload)
            VALUES (?, ?, ?, ?, ?, ?, ?)""",
                   (identifier, record["case_id"], record["case_version"], based_on,
                    request_id, request_body, canonical_json(record)))
        return record

    def _seed_case(self, db, case):
        validate_case(case)
        if any(case[key] != value for key, value in CASE_FLAGS.items()):
            raise ValidationError("Source fixtures must remain synthetic, unreviewed, ineligible demos.")
        snapshot = read_json(canonical_json(case))
        previous = db.execute("SELECT payload FROM case_versions WHERE case_id = ? AND version_label = ?",
                              (case["case_id"], case["version"])).fetchone()
        if previous:
            record = read_json(previous[0])
            if record["snapshot"] != snapshot:
                raise Conflict("A source fixture changed an existing case version. Restore it or give it a new version.")
            return record
        latest_row = db.execute("SELECT payload FROM case_versions WHERE case_id = ? ORDER BY rowid DESC LIMIT 1",
        ...
        return self._insert_version(db, snapshot, identifier, None,
    ...
    def _seed_families(self, db):
        ...
        for family in families:
            ...
            base_row = db.execute("SELECT payload FROM case_versions WHERE case_id = ? AND version_label = ?",
                                  (family.get("base_case_id"), family.get("base_version"))).fetchone()
            if base_row is None:
            ...
            if (family["family_id"] in family_ids or base["case_id"] in base_ids
            ...
            record = {
                "family_id": family["family_id"], "title": family["title"],
                "factor_label": family["factor_label"], "base_version_id": base["version_id"],
                "variant_version_id": variant["version_id"], "held_constant": family["held_constant"],
                "changed_fact": {"label": family["fact_label"], "base_value": family["base_value"],
                                 "variant_value": family["variant_value"]},
            }
            payload = canonical_json(record)
            existing = db.execute("SELECT payload FROM case_families WHERE id = ?",
                                  (record["family_id"],)).fetchone()
            if existing:
                if existing[0] != payload:
                    raise Conflict("A source family changed an existing immutable comparison.")
            else:
                db.execute("INSERT INTO case_families(id, payload) VALUES (?, ?)",
                           (record["family_id"], payload))

    def overview(self):
        with self.connection() as db:
            db.execute("BEGIN")
            versions = [_with_review(db, read_json(row[0])) for row in
                        db.execute("SELECT payload FROM case_versions ORDER BY rowid")]
            reviews = [read_json(row[0]) for row in
                       db.execute("SELECT payload FROM case_reviews ORDER BY rowid")]
            families = [read_json(row[0]) for row in
                        db.execute("SELECT payload FROM case_families ORDER BY rowid")]
            return {"versions": versions, "reviews": reviews, "families": families}

    def cases(self):
        with self.connection() as db:
            db.execute("BEGIN")
            ids = [row[0] for row in db.execute("SELECT case_id FROM case_versions GROUP BY case_id ORDER BY MIN(rowid)")]
    ...
    def resolve(self, db, case_id=None, version_id=None):
        ...
        if version_id is not None:
            ...
            version = _get(db, "case_versions", version_id)
        else:
        ...
        return {"case_snapshot": snapshot, "case_version_id": version["version_id"],
    ...
    def add_review(self, body):
        ...
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            duplicate = _retry(db, "case_reviews", body["request_id"], request_body)
            if duplicate:
                return duplicate, True
            _get(db, "case_versions", body["version_id"])
            ...
            record = {**body, "review_id": str(uuid4()), "reviewer_code": reviewer,
                      "recorded_at": now()}
            db.execute("""INSERT INTO case_reviews
                (id, version_id, supersedes_id, request_id, request_body, payload)
                VALUES (?, ?, ?, ?, ?, ?)""",
                       (record["review_id"], record["version_id"], expected,
                        body["request_id"], request_body, canonical_json(record)))
        return record, False
    ...
    def add_version(self, body):
        ...
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            duplicate = _retry(db, "case_versions", body["request_id"], request_body)
            if duplicate:
                return _with_review(db, duplicate), True
            prior = _get(db, "case_versions", body["based_on_version_id"])
            ...
            if snapshot["case_id"] != prior["case_id"] or snapshot["family_id"] != prior["snapshot"]["family_id"]:
            ...
            if db.execute("SELECT 1 FROM case_versions WHERE case_id = ? AND version_label = ?",
                          (prior["case_id"], body["version"])).fetchone():
                raise Conflict("That case version already exists. Choose a new version label.")
            snapshot.update(CASE_FLAGS)
            record = self._insert_version(db, snapshot, str(uuid4()), editor,
                                          body["change_note"], prior["version_id"],
                                          body["request_id"], request_body)
