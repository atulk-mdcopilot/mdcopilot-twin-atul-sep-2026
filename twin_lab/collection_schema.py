...
PROTOCOL_V2_FIELDS = (PROTOCOL_FIELDS - LEGACY_POLICY_FIELDS) | {
    "protocol_schema_version",
    "governance_id",
}
PLANNING_FIELDS = ("owner_code", "physician_codes", "backup_owner_code")
GOVERNANCE_FIELDS = (
    "owner_code",
    "physician_codes",
    "consent_statement",
...
def validate_protocol(body):
    v2 = isinstance(body, dict) and body.get("protocol_schema_version") == "2.0"
    exact_keys(body, PROTOCOL_V2_FIELDS if v2 else PROTOCOL_FIELDS)
    ...
    for key in ("notes",) if v2 else ("consent_statement", "consent_version", "notes"):
        text_field(
            body[key],
            key.replace("_", " "),
            limit=12000 if key == "consent_statement" else 5000,
            required=False,
        )
    ...
    for key in () if v2 else ("retention_days", "backup_retention_days"):
        value = body[key]
