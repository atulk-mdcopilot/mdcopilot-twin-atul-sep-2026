...
from .schemas import (Conflict, NotFound, ValidationError, canonical_json, exact_keys,
                      now, text_field, validate_code, validate_id)
...
def expiry(anchor, governance):
    """The approved policy is frozen at original submission, never at correction."""
    policy = governance["retention"]
    close = datetime.combine(datetime.fromisoformat(governance["operator"]["pilot_close_date"]).date(),
                             time.max, tzinfo=timezone.utc)
    return min(instant(anchor) + timedelta(days=policy["response_days"]),
               close + timedelta(days=policy["after_close_days"])).isoformat()
...
class Lifecycle:
    def __init__(self, store):
        ...
        with store.connection() as db:
            ...
            initialize_journals(self, db)
            db.execute("CREATE TABLE IF NOT EXISTS response_lifecycle (id TEXT PRIMARY KEY, payload TEXT NOT NULL)")
            ...
            db.execute("CREATE TABLE IF NOT EXISTS managed_exports (id TEXT PRIMARY KEY, payload TEXT NOT NULL)")
            for table in ("response_lifecycle", "lifecycle_events", "managed_exports"):
    ...
    def events(self, db):
        ...
        events = {item["event_id"]: item for item in self.rows(db, "lifecycle_events")}
        events.update({item["event"]["event_id"]: item["event"] for item in restriction_entries(self.ledger_path.parent)})
    ...
    def root_for(self, db, response_id):
        ...
        while response_id not in visited:
            ...
            response = self.store._get(db, "responses", response_id)
    ...
    def attach(self, db, response, governance=None, permission_receipt_id=None):
        ...
        metadata = {"root_response_id": response["response_id"],
                    "retention_anchor_at": response["submitted_at"], "expires_at": None,
                    "permission_status": "fabricated_qa" if response.get("capture_mode") == "fabricated_qa"
                    else "unknown_legacy", "permission_receipt_id": permission_receipt_id,
                    "governance_id": None, "policy": None}
        if governance is not None and permission_receipt_id:
            metadata.update(permission_status="recorded", governance_id=governance["governance_id"],
                            expires_at=expiry(response["submitted_at"], governance),
                            policy=dict(governance["retention"]))
        db.execute("INSERT INTO response_lifecycle(id,payload) VALUES (?,?)",
                   (response["response_id"], canonical_json(metadata)))
        return metadata

    def metadata(self, db, response_id):
        root = self.root_for(db, response_id)
        row = db.execute("SELECT payload FROM response_lifecycle WHERE id=?", (root["response_id"],)).fetchone()
        if row:
            return json.loads(row[0])
        return {"root_response_id": root["response_id"], "retention_anchor_at": root["submitted_at"],
                "expires_at": None, "permission_status": "unknown_legacy", "permission_receipt_id": None,
                "governance_id": None, "policy": None}
    ...
    def is_blocked(self, db, physician_code, receipt_id=None):
        ...
        return any(event["kind"] == "withdrawal" and
                   event["physician_code"] == physician_code
    ...
    def restriction(self, db, response_id, at=None):
        ...
        if any(item["kind"] == "hold" and item["hold_id"] not in released and
               identifier in item["root_ids"] for item in events):
            return "legal_hold"
        if self.is_blocked(db, root["physician_code"]):
            return "withdrawn"
        metadata = self.metadata(db, identifier)
        if metadata["expires_at"] and instant(metadata["expires_at"]) <= instant(at or self.clock()):
            return "expired"
        return None
    ...
    def _event(self, db, kind, body, payload):
        ...
        previous = db.execute("SELECT request_body,payload FROM lifecycle_events WHERE request_id=?",
        ...
        journal = next((item for item in restriction_entries(self.ledger_path.parent)
                        if item["request_id"] == body["request_id"]), None)
        ...
        event = journal["event"] if journal else dict(payload, kind=kind, request_id=body["request_id"],
                                                       event_id=str(uuid4()), recorded_at=self.clock())
        if not journal:
            append_ledger(self.ledger_path.parent / "lifecycle-events.jsonl",
                          {"request_id": body["request_id"], "request_body": frozen_body, "event": event})
        db.execute("INSERT INTO lifecycle_events(id,request_id,kind,request_body,payload) VALUES (?,?,?,?,?)",
                   (event["event_id"], body["request_id"], kind,
                    frozen_body, canonical_json(event)))
        return event, False
    ...
    def withdraw(self, body):
        ...
        with self.store.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            roots = sorted({self.root_for(db, row["response_id"])["response_id"]
                            for row in self.rows(db, "responses") if row["physician_code"] == code})
            return self._event(db, "withdrawal", body, {"withdrawal_id": body["request_id"],
                "physician_code": code, "actor_code": actor, "root_ids": roots,
                "received_at": self.clock()})

    def verify_withdrawal(self, body):
        exact_keys(body, {"request_id", "withdrawal_id", "actor_code", "verification_attested", "withdrawal_days"})
        validate_id(body["request_id"])
        validate_id(body["withdrawal_id"])
        actor = validate_code(body["actor_code"])
        days = body["withdrawal_days"]
        if body["verification_attested"] is not True or (days is not None and
                (type(days) is not int or not 1 <= days <= 3650)):
            raise ValidationError("Reasonable verification must be attested; provide valid days or null for policy.")
        with self.store.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            if db.execute("SELECT 1 FROM lifecycle_events WHERE request_id=?", (body["request_id"],)).fetchone():
                return self._event(db, "verification", body, {})
            events = self.events(db)
            withdrawal = next((item for item in events if item["kind"] == "withdrawal" and
                               item["withdrawal_id"] == body["withdrawal_id"]), None)
            if not withdrawal:
                raise NotFound("Withdrawal receipt was not found.")
            existing = next((item for item in events if item["kind"] == "verification" and
                             item["withdrawal_id"] == body["withdrawal_id"]), None)
            ...
            for root_id in withdrawal["root_ids"]:
                ...
                if policy_days is None:
                    raise ValidationError("Legacy records need an explicitly chosen withdrawal deadline.")
            ...
            return self._event(db, "verification", body, {"withdrawal_id": body["withdrawal_id"],
                "actor_code": actor, "verification_attested": True, "verified_at": self.clock(),
                "deadlines": deadlines, "unavailable_root_ids": unavailable,
    ...
    def hold(self, body):
        ...
        with self.store.connection() as db:
            ...
            roots = sorted({self.root_for(db, validate_id(identifier))["response_id"] for identifier in body["response_ids"]})
            if any(self.restriction(db, root) == "disposal_recorded" for root in roots):
                raise Conflict("Disposal is already recorded for this response; recover the pending disposal first.")
            return self._event(db, "hold", body, {"hold_id": body["request_id"], "root_ids": roots,
                "response_ids": list(body["response_ids"]), "actor_code": actor,
                "authority_record": body["authority_record"], "created_at": self.clock()})
    ...
    def release_hold(self, body):
        ...
        with self.store.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            if not any(item["kind"] == "hold" and item["hold_id"] == body["hold_id"] for item in self.events(db)):
                raise NotFound("Hold was not found.")
            return self._event(db, "hold_release", body, {key: body[key] for key in body if key != "request_id"})
    ...
    def overview(self):
        ...
        with self.store.connection() as db:
            db.execute("BEGIN")
            grouped = {}
            for response in self.rows(db, "responses"):
                metadata = self.metadata(db, response["response_id"])
                grouped.setdefault(metadata["root_response_id"], dict(metadata, response_ids=[]))["response_ids"].append(response["response_id"])
        ...
        return {"records": records, "withdrawals": [item for item in events if item["kind"] == "withdrawal"],
                ...
                **copies, "disposal": plan_disposal(self)}
