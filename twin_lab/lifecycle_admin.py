...
def administrative_review(lifecycle, db, at):
    ...
    versions = {row["governance_id"]: row for row in read_records(db, "governance_versions")}
    responses = read_records(db, "responses")
    ...
    def due(version, field):
        ...
        return datetime.combine(
            datetime.fromisoformat(close).date(), time.max, tzinfo=timezone.utc
        ) + timedelta(days=days)

    permissions = []
    for receipt in read_records(db, "permission_receipts"):
    ...
    for identifier, version in versions.items():
        deadlines = [
            due(version, field)
            for field in ("permission_after_close_days", "audit_after_close_days")
        ]
        if all(deadlines) and max(deadlines) <= instant(at):
            governance.append(
                {"governance_id": identifier, "expires_at": max(deadlines).isoformat()}
            )
    metadata = {row["root_response_id"]: row for row in read_records(db, "response_lifecycle")}
    audit = []
    for event in lifecycle.events(db):
        roots = event.get("root_ids", list(event.get("deadlines", {})))
        deadlines = [
            due(
                versions.get(metadata.get(root, {}).get("governance_id"), {}),
    ...
    return {
        "overdue_permissions": permissions,
        "overdue_governance": governance,
