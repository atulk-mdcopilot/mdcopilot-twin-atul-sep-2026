...
from .schemas import Conflict, Record, ValidationError, canonical_json, validate_code, validate_id
...
def initialize_journals(lifecycle, db):
    initialized = db.execute(
        "SELECT 1 FROM lifecycle_state WHERE id='journals_initialized'"
    ).fetchone()
    for name in ("deletion-ledger.jsonl", "lifecycle-events.jsonl"):
        ...
        if initialized and not path.is_file():
            raise RuntimeError(
                "An authoritative lifecycle journal is missing. Restore it before opening this database."
            )
        if not path.exists():
...
def ledger_entries(path):
    ...
    try:
        ...
        raise RuntimeError(
            "Deletion ledger is invalid; recover the authoritative ledger before continuing."
        ) from exc
...
def restriction_entries(directory):
    ...
    try:
        ...
        for item in entries:
            ...
            kind = event["kind"]
            if (
                kind not in ("withdrawal", "verification", "hold", "hold_release")
                or body["event_kind"] != kind
            ):
                raise ValueError
            if kind == "withdrawal":
        ...
        raise RuntimeError(
            "The authoritative lifecycle journal is invalid; recover it before continuing."
        ) from exc
...
def inventory(lifecycle, db, at=None):
    ...
    roots = {
        lifecycle.root_for(db, row[0])["response_id"]
        for row in db.execute("SELECT id FROM responses")
    }
    restrictions = {root: lifecycle.restriction(db, root, at) for root in roots}
    ledger = ledger_entries(lifecycle.ledger_path)
    disposed: dict[str, datetime] = {}
    for item in ledger:
    ...
    for kind, table in (("exports", "managed_exports"), ("backups", "local_backups")):
        copies = []
        for original in read_records(db, table):
            item = dict(original)
            source_ids = item.get("source_root_ids", [])
            item["held"] = any(restrictions.get(root) == "legal_hold" for root in source_ids)
            item["restricted_sources"] = [
                root for root in source_ids if restrictions.get(root) or root in disposed
            ]
            ...
            item["usable"] = bool(
                item["present"] and not item["expired"] and not item["restricted_sources"]
            )
            copies.append(item)
        result[kind] = copies
    result["unmanaged_copy_filenames"] = {
        kind: sorted(
            path.name
            for path in (lifecycle.store.db_path.parent / kind).glob("*")
            if path.is_file()
            and not path.name.startswith(".")
            and path.name not in {item["filename"] for item in result[kind]}
        )
...
def managed_export(lifecycle, payload):
    ...
    with lifecycle.store.connection() as db:
        ...
        metadata = {
            lifecycle.metadata(db, item["response_id"])["root_response_id"]: lifecycle.metadata(
                db, item["response_id"]
            )
            ...
            if item["expires_at"]:
                deadlines.extend(
                    (
                        ...
                        instant(created) + timedelta(days=item["policy"]["export_days"]),
                    )
                )
        manifest = {
            ...
            "created_at": created,
            "filename": path.name,
            "source_root_ids": sorted(metadata),
        ...
        try:
            ...
            db.execute(
                "INSERT INTO managed_exports(id,payload) VALUES (?,?)",
                (identifier, canonical_json(manifest)),
            )
...
def plan_disposal(lifecycle, at=None, db=None):
    ...
    withdrawals = {
        event["withdrawal_id"]: event for event in events if event["kind"] == "withdrawal"
    }
    ...
    for event in events:
        if event["kind"] == "verification":
            ...
            if withdrawal is None:
                raise RuntimeError(
                    "Withdrawal verification has no matching receipt; recover the authoritative journals."
                )
            ...
            for root in withdrawal["root_ids"]:
                ...
                days = (
                    metadata["policy"]["withdrawal_days"]
                    if metadata["policy"]
                    else event.get("withdrawal_days")
                )
                if days is None:
    ...
    for root in roots:
        ...
        if restriction == "legal_hold":
            ...
            continue
        root_deadlines = [
            instant(value) for value in (metadata["expires_at"], verified.get(root)) if value
        ]
        if restriction == "disposal_recorded" or (
            root_deadlines and min(root_deadlines) <= instant(timestamp)
        ):
            eligible.append(root)
        elif not root_deadlines:
            unknown.append(root)
    copies = inventory(lifecycle, db, timestamp)
    exports = [
        row["export_id"]
        for row in copies["exports"]
        if row["present"]
        and not row["held"]
        and (row["expired"] or set(row["source_root_ids"]) & set(eligible))
    ]
    backups = [
        row["backup_id"]
        for row in copies["backups"]
        if row["present"] and not row["held"] and row["expired"]
    ]
    plan = {
        "plan_schema_version": "1.0",
        "as_of": timestamp,
        "eligible_root_ids": sorted(eligible),
        "held_root_ids": sorted(held),
        "unknown_policy_root_ids": sorted(unknown),
        "export_ids": sorted(exports),
        "backup_ids": sorted(backups),
    ...
    with lifecycle.store.connection() as db:
        ...
        if roots or plan["export_ids"] or plan["backup_ids"]:
            append_ledger(
                lifecycle.ledger_path,
                {
                    "entry_id": str(uuid4()),
                    "root_ids": roots,
                    "deleted_at": lifecycle.clock(),
                    "actor_code": actor,
                    "plan_confirmation": plan["confirmation"],
                    "status": "authorized",
                    "export_ids": plan["export_ids"],
        ...
        for kind, identifier in (("exports", "export_id"), ("backups", "backup_id")):
            for manifest in copies[kind]:
                if (
                    manifest[identifier]
                    in plan["export_ids" if kind == "exports" else "backup_ids"]
                ):
        ...
        return {
            "disposed_root_ids": roots,
            "removed_copy_ids": removed,
