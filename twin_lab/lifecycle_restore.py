...
def purge_roots(db, root_ids):
    ...
    records = [(row[0], row[1], json.loads(row[2])) for row in
               db.execute("SELECT id,presentation_id,payload FROM responses ORDER BY rowid")]
    selected = set(root_ids)
    changed = True
    while changed:
        additions = {identifier for identifier, _, payload in records
                     if payload.get("supersedes_response_id") in selected} - selected
        changed = bool(additions)
        selected.update(additions)
    presentation_ids = {presentation_id for identifier, presentation_id, _ in records if identifier in selected}
    presentation_ids.update(row[0] for row in db.execute("SELECT id,payload FROM presentations")
...
def restore_snapshot(source, destination, ledger_path):
    ...
    if destination.exists() or destination.is_symlink():
        raise ValidationError("Restore requires a new destination; existing files cannot be replaced.")
    if not source.is_file() or not ledger_path.is_file():
        raise ValidationError("A backup and the authoritative external deletion ledger are required.")
    ...
    descriptor, name = tempfile.mkstemp(prefix=".restore-", suffix=".sqlite3", dir=destination.parent)
    os.close(descriptor)
    temporary = Path(name)
    try:
        ...
        try:
            ...
            with restored:
                ...
                restored.execute("""CREATE TABLE IF NOT EXISTS lifecycle_events
                ...
                for item in restrictions:
                    event = item["event"]
                    existing = restored.execute("SELECT request_body FROM lifecycle_events WHERE request_id=?",
                    ...
                    if not existing:
                        restored.execute("INSERT INTO lifecycle_events VALUES (?,?,?,?,?)",
                            (event["event_id"], item["request_id"], event["kind"], item["request_body"],
                             json.dumps(event, ensure_ascii=False)))
                restored.execute("CREATE TABLE IF NOT EXISTS restore_barriers (id TEXT PRIMARY KEY,payload TEXT NOT NULL)")
                barrier = {"restore_id": str(uuid4()), "restored_at": now()}
                restored.execute("INSERT INTO restore_barriers VALUES (?,?)",
        ...
        for name in ("deletion-ledger.jsonl", "lifecycle-events.jsonl"):
            authoritative = ledger_path if name == "deletion-ledger.jsonl" else ledger_path.parent / name
            target = destination.parent / name
            contents = authoritative.read_bytes()
            if target.exists():
                if target.read_bytes() != contents:
                    raise ValidationError("The destination contains different lifecycle journals. Use a new restore directory.")
            else:
        ...
        return {"destination": str(destination), "ledger_entries_applied": len(entries),
                "restriction_events_applied": len(restrictions),
                "removed_response_ids": removed["response_ids"], "integrity_check": "ok",
