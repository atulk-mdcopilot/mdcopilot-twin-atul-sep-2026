...
def main(argv=None):
    ...
    plan_parser = commands.add_parser("plan", help="Preview eligible disposal without deleting anything")
    plan_parser.add_argument("--output", type=Path, required=True)
    apply_parser = commands.add_parser("apply", help="Apply a reviewed plan; stop the application first")
    apply_parser.add_argument("--plan", type=Path, required=True)
    apply_parser.add_argument("--actor-code", required=True)
    apply_parser.add_argument("--confirm", required=True, help="Exact confirmation digest from the reviewed plan")
    restore_parser = commands.add_parser("restore", help="Restore into a new location with current lifecycle journals")
    ...
    try:
        if args.command == "restore":
            result = restore_snapshot(args.source, args.destination, args.data_dir / "deletion-ledger.jsonl")
        else:
            ...
            if args.command == "plan":
                ...
                if not isinstance(plan, dict) or plan.get("confirmation") != args.confirm:
                    raise ValidationError("Confirmation must match the exact reviewed disposal plan.")
        ...
        parser.exit(2, "Maintenance stopped: local storage or journal verification failed. No completion is claimed.\n")
