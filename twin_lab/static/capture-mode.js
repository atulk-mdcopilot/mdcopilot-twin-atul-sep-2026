...
export function initCaptureMode(container, {onOpenAssignment, onModeChange}) {
  ...
  function renderHuman() {
    ...
    if (!governance?.readiness.actual_physician_capture_enabled) return;
  ...
  async function refresh() {
    try {
      ...
      if (!permissionEditor?.protectedDraft() && !correction) renderHuman();
    } catch (error) {
      governance = null;
      mode.options[1].disabled = true;
  ...
  function context(body, original = null) {
    ...
    if (!governance?.readiness.actual_physician_capture_enabled) throw new Error("Actual physician participation is disabled. Review the Governance requirements first.");
