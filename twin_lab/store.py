...
from .schemas import (SCHEMA_VERSION, Conflict, NotFound, ValidationError,
                      canonical_json, now, read_json, snapshot_hash,
                      validate_case, validate_id, validate_values)
...
GOVERNANCE_REFS = ("capture_mode", "governance_id", "permission_receipt_id",
                   "permission_version", "permission_sha256")
...
class Store:
    def __init__(self, db_path, fixtures_path=FIXTURES):
        ...
        with self.connection() as db:
            ...
            """)
            for table in ("presentations", "responses"):
        ...
        self.catalog.initialize(cases, include_families=Path(fixtures_path).resolve() == FIXTURES.resolve())
    ...
    def _get(db, table, identifier):
        # Only constant internal table names reach this helper.
        row = db.execute(f"SELECT payload FROM {table} WHERE id = ?", (identifier,)).fetchone()
        if row is None:
            raise NotFound("The requested local record was not found.")
        return json.loads(row[0])

    @staticmethod
    def _require_latest(db, identifier):
        if db.execute("SELECT 1 FROM responses WHERE supersedes_id = ?", (identifier,)).fetchone():
            raise Conflict("This response already has a correction. Review and correct the latest revision.")

    def present(self, case_id=None, supersedes_response_id=None, version_id=None,
                assignment_id=None, capture_mode=None, permission_receipt_id=None,
                qa_acknowledged=False):
        if sum(value is not None for value in (case_id, supersedes_response_id, version_id, assignment_id)) != 1:
            raise ValidationError("Choose one case, exact version, assignment, or response to correct.")
        ...
        if capture_mode == "fabricated_qa" and not qa_acknowledged:
            raise ValidationError("Confirm that these are fabricated software-test answers before opening the case.")
        if capture_mode != "fabricated_qa" and qa_acknowledged:
            raise ValidationError("The fabricated-answer acknowledgment does not apply to this mode.")
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            previous = None
            if supersedes_response_id is not None:
                validate_id(supersedes_response_id)
                previous = self._get(db, "responses", supersedes_response_id)
                ...
                if capture_mode != previous_mode or permission_receipt_id != previous.get("permission_receipt_id"):
                    raise Conflict("A correction must preserve the original capture mode and permission reference.")
                resolved = {"case_snapshot": previous["case_snapshot"],
            ...
            if capture_mode == "legacy_unclassified" and previous is not None:
                governance = {"capture_mode": "legacy_unclassified", "governance_id": None,
                ...
                governance = self.governance.authorize(
                    db, capture_mode, permission_receipt_id,
                    code=previous["physician_code"] if previous else resolved.get("assigned_physician_code"),
                    assignment_id=resolved.get("assignment_id"),
                    protocol_id=resolved.get("protocol_id"), version_id=resolved.get("case_version_id"))
            snapshot = resolved["case_snapshot"]
            presentation = {
                "presentation_id": str(uuid4()), "presented_at": now(),
            ...
            db.execute("INSERT INTO presentations(id, payload) VALUES (?, ?)",
    ...
    def submit(self, presentation_id, values):
        ...
        with self.connection() as db:
            db.execute("BEGIN IMMEDIATE")
            existing = db.execute("SELECT payload FROM responses WHERE presentation_id = ?",
            ...
            if existing:
                ...
                if response["original_values"] != original:
                    raise Conflict("This presentation was already saved. Review it and use an explicit correction.")
                if response.get("capture_mode") == "physician_demo":
                    self.governance.authorize(
                        db, "physician_demo", response["permission_receipt_id"],
                        code=response["physician_code"], assignment_id=response["assignment_id"],
                        protocol_id=response["protocol_id"], version_id=response["case_version_id"])
                return response, True
            presentation = self._get(db, "presentations", presentation_id)
            mode = presentation.get("capture_mode")
            if mode is None:
                raise Conflict("This case was opened before the governance update. Reopen it and choose the capture mode.")
            self.lifecycle.require_capture(db, normalized["physician_code"], presentation.get("permission_receipt_id"))
            ...
            if supersedes is not None:
                previous = self._get(db, "responses", supersedes)
            ...
            if mode != "legacy_unclassified":
                verified = self.governance.authorize(
                    db, mode, presentation.get("permission_receipt_id"), code=normalized["physician_code"],
                    assignment_id=presentation.get("assignment_id"),
                    protocol_id=presentation.get("protocol_id"), version_id=presentation.get("case_version_id"))
                if any(verified.get(key) != presentation.get(key) for key in GOVERNANCE_REFS):
                    raise Conflict("Permission changed after this case was opened. Reopen the case.")
            elif supersedes is None:
                raise Conflict("Unclassified status is reserved for corrections of historical responses.")
            case = presentation["case_snapshot"]
            response = {
                "response_id": str(uuid4()), "presentation_id": presentation_id,
                "physician_code": normalized["physician_code"],
                "case_id": case["case_id"], "case_family": case["family_id"],
                "case_version": case["version"], "response_schema_version": SCHEMA_VERSION,
                "case_snapshot": case, "snapshot_sha256": presentation["snapshot_sha256"],
                "presented_at": presentation["presented_at"], "submitted_at": now(),
                "original_values": original, "normalized_values": normalized,
                "ai_advice_shown": False, "collection_purpose": case["collection_purpose"],
                "case_review_status": case["review_status"], "eligible_for_study": False,
                ...
                **{key: presentation.get(key) for key in GOVERNANCE_REFS},
                "training_allowed": False, "research_reuse_allowed": False,
                "public_release_allowed": False,
            }
            payload = canonical_json(response)
            db.execute("INSERT INTO responses(id, presentation_id, supersedes_id, payload) VALUES (?, ?, ?, ?)",
                       (response["response_id"], presentation_id, supersedes, payload))
            governance_record = (self.governance.get(db, response["governance_id"])
                                 if response["governance_id"] else None)
            self.lifecycle.attach(db, response, governance_record, response["permission_receipt_id"])
        return json.loads(payload), False

    def responses(self):
        with self.connection() as db:
            db.execute("BEGIN")
            return [json.loads(row[0]) for row in db.execute("SELECT payload FROM responses ORDER BY rowid")
    ...
    def export(self):
        ...
        with self.connection() as db:
            db.execute("BEGIN")
            tables = {table: self.collection._records(db, table) for table in
                      ("responses", "case_versions", "case_reviews", "case_families",
                       "collection_protocols", "case_assignments")}
            tables["responses"] = [item for item in tables["responses"]
        ...
        return {"export_schema_version": SCHEMA_VERSION, "exported_at": now(),
                "application": "Twin Lab v0.1", "synthetic_only": True,
                "response_count": len(responses), "responses": responses, **tables}
